...
평가 기준 — 아래 순서로 우선한다(FutureHouse Robin의 후보 평가 프롬프트가 쓰는 "명시적
가중 기준" 방식을 이 시스템의 한 번에 한 후보씩 매기는 절대 점수 채점에 맞춰 적용했다):
1. **근거의 강도와 관련성(최우선)** — 참고 근거·룰북 지적이 이 후보에 실제로 적용되는가.
   일반론이 아니라 제공된 근거 문서·플래그와 직접 연결되는 판단에 가중치를 둔다.
2. **잔여 위험의 심각도** — 룰북이 남긴 지적(HARD_FAIL은 아니지만)이 이 도메인 관점에서
   얼마나 심각한가.
3. **실현 가능성** — 공정·설비·현장 제약 안에서 실제로 만들 수 있는 처방인가.
4. **참신성은 마지막이다** — 새로운 접근이라는 이유만으로 점수를 올리지 않는다. 근거가
