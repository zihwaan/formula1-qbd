...
서버가 읽은 맥락 ┘   (LLM 해석 +            │           · 설계 실행 (폼을 채워 보여 준 뒤 시작)
...
LLM이 응답하지 않으면 규칙 기반 해석기가 글에서 값만 읽어 같은 카드를 만들고, 그 사실을 표시합니다. LLM이 되묻기만 했는데 글에서 행동이 분명히 읽히면(예: "압축력은 몰라요") 규칙 해석이 바닥을 받쳐 카드를 냅니다. 화면에서는 오른쪽 아래 **입력 에이전트** 버튼으로 열고, 휴대폰에서는 바닥 시트로 뜹니다.
...
**입력 에이전트로도 같은 흐름을 탈 수 있습니다.** 카드를 누르는 대신 오른쪽 아래 **입력 에이전트**에 "성인용 이부프로펜 정제로 설계해 줘"라고 쓰면, 용량을 되묻고("1회 200 mg") 구조식을 내장 사전에서 찾아 출처와 함께 실행 카드를 냅니다. 설계가 끝나면 에이전트가 먼저 권고 후보와 가장 가벼운 남은 요청을 알려 주고, 측정값을 말로 전하면 제출 카드로 바꿔 줍니다.
...
AI 단계(후보 설계·심사·가설)에는 `GROQ_API_KEY`(무료) 또는 `ANTHROPIC_API_KEY`가 필요합니다. 키가 없거나 응답이 없으면 그 단계는 **결과를 만들지 않고 "응답 없음"으로 표시**합니다 — 틀에 박힌 처방이나 점수로 채우지 않습니다. 요청 해석·재설계 지시·입력 에이전트의 값 읽기는 규칙 기반으로도 처리되고, 규칙 검사·분자 계산·개발 스튜디오의 통계 엔진과 판정은 키 없이 그대로 동작합니다.
