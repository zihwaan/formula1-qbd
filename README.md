...
실제 실험은 며칠에서 몇 주가 걸리므로, 이 전 과정은 **한 번의 대화로 끝나지 않고 데이터베이스에 상태로 남습니다.** 서버가 재시작되거나 담당자가 바뀌어도 "지금 어느 단계고 다음에 누가 무엇을 해야 하는지"는 사라지지 않고, 용출이 미달했다고 그 자리에서 바로 처방을 고치지도 않습니다 — 원인을 가려낼 확인시험을 먼저 거쳐 **확정된 원인만** 새 버전의 후보로 반영합니다. 이때 "서로 다른 원인 후보를 진짜로 여러 개 생각하게" 만드는 AI 지시문 작성법은, 실제로 신약 후보를 찾아낸 것으로 유명한 연구용 AI 시스템 **Robin**(FutureHouse, Nature 2026)의 공개 프롬프트를 직접 읽고 가져왔습니다 — 무엇을 어떻게 가져왔는지는 [9장](#9-만든-뒤의-절반--다음-실험을-지시한다) 끝에 있습니다.
