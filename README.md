...
실제 실험은 며칠에서 몇 주가 걸리므로, 이 전 과정은 **한 번의 대화로 끝나지 않고 데이터베이스에 상태로 남습니다.** 서버가 재시작되거나 담당자가 바뀌어도 "지금 어느 단계고 다음에 누가 무엇을 해야 하는지"는 사라지지 않고, 용출이 미달했다고 그 자리에서 바로 처방을 고치지도 않습니다 — 원인을 가려낼 확인시험을 먼저 거쳐 **확정된 원인만** 새 버전의 후보로 반영합니다 ([9장](#9-만든-뒤의-절반--다음-실험을-지시한다)).
...
앞서 언급한 "lab-in-the-loop"이라는 말은 FutureHouse·옥스퍼드대·포덤대 연구진이 발표한 다중 에이전트 시스템 **Robin**([Nature, 2026](https://www.nature.com/articles/s41586-026-10652-y) · [arXiv:2505.13400](https://arxiv.org/abs/2505.13400))에서 왔습니다. Robin은 문헌 검색 에이전트(Crow·Falcon)와 데이터 분석 에이전트(Finch)를 엮어 가설 생성 → 실험 제안 → 결과 해석 → 가설 갱신을 반자동으로 도는 시스템으로, 노년 황반변성 치료 후보(ripasudil)를 실제로 찾아낸 사례로 발표됐습니다. FutureHouse는 이 시스템의 **프롬프트 원문을 공개**했습니다(GitHub `Future-House/robin`, `robin/prompts.py`, 835줄). 이번에 그 프롬프트를 실제로 읽고, 우리 진단·심사·설계 프롬프트에 두 가지를 구체적으로 반영했습니다 — 아이디어 차용이 아니라 **문장 단위로 대조하며 가져온 것**만 적습니다.
...
화면 구성은 이렇습니다. 왼쪽에 분자 구조와 특성값, 그리고 구조 패턴 검사. 가운데 위에 에이전트 그래프, 그 아래에 실행에 맞춰 흐르는 아키텍처 해설, 맨 아래에 실행 트레이스. 오른쪽에 후보 처방과 합의 결과, 그 아래에 **근거 충족 게이트**(확인시험 요청·결과 입력·승인)와 **장기 실행 작업함**(프로토콜 승인·배치 등록·결과 확인·진단·원인 확정을 며칠에 걸쳐 이어 가는 패널) 패널이 차례로 옵니다 — 화면 순서가 곧 실행 순서입니다. 트레이스의 규칙 이름을 클릭하면 **원본 표의 행과 출처 문헌**이 열립니다.
