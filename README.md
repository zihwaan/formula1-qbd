...
**바로 써 보기 → <https://zihwan.com/formula1>** (비밀번호 또는 게스트로 접속 — 게스트는 무료 모델만)
...
**심사 점수에는 검증된 인용이 필요합니다.** 심사관은 인용을 DOI·PMID로만 할 수 있고, 인용 후보로는 이 약에 대해 Europe PMC가 돌려준 실제 문헌과 룰북 인용 등록부(`citation_registry.csv` — 룰북에 적힌 식별자를 Crossref·NCBI로 조회해 통과한 것)가 주어집니다. 목록 밖 식별자는 그 자리에서 Crossref·NCBI로 조회해 실재할 때만 인정하고, 검증된 인용이 하나도 없으면 그 점수는 **무효**로 합의에서 빠집니다("출처 없는 판단은 쓰지 않는다"는 원칙을 심사 점수에도 적용). 응답이 없으면 간격을 두고 두 번 더 시도하고, 그래도 없으면 "점수 없음"으로 표시합니다. 모델의 내부 추론(reasoning 채널)은 화면에 흘리지 않습니다.
...
측정값 제출은 **LLM보다 규칙이 먼저** 잡습니다 — 열린 설계가 있고, 글에 측정 이름과 숫자(예: "DSC 측정 결과: Tm 317 도. 실험 용해도는 0.00005 mg/mL.")가 있으며 새 설계를 요청하는 말이 없으면 곧바로 제출 카드가 됩니다. 이 판단을 LLM에 맡기면 새 설계 요청으로 오분류되어 전체가 다시 돌 수 있기 때문입니다. 제출 카드에서 근거 등급을 고르고(기본: 사용자 진술), 새 설계가 시작되면 이전 설계에 묶인 카드는 잠깁니다. LLM이 응답하지 않으면 규칙 기반 해석기가 글에서 값만 읽어 같은 카드를 만들고, 그 사실을 표시합니다. LLM이 되묻기만 했는데 글에서 행동이 분명히 읽히면(예: "압축력은 몰라요") 규칙 해석이 바닥을 받쳐 카드를 냅니다. 화면에서는 **탭 바로 아래, 가장 먼저 보이는 주 입력 창**이 입력 에이전트이고(두 탭 공용), 폼에 직접 값을 넣는 것은 그 아래 **직접 입력**에 접힌 보조 경로입니다.
...
염 형태 SMILES(예: 베실산 암로디핀)가 들어오면 가장 큰 유기 조각을 parent로 확정하고, **분자 특성값·구조 패턴·용해도 예측을 모두 parent로만** 계산합니다 — 염 전체로 계산하면 짝이온의 질량·극성이 섞여 Ro5·Veber 경고가 잘못 켜집니다(베실산 암로디핀: 염 MW 567.1 → parent 408.9, TPSA 154.3 → 99.9). 염 정보(짝이온, 염/유리염기 분자량비)는 따로 보존해 함량 환산에 씁니다. 약물 이름도 구조로 찾은 PubChem 표제명으로 정규화해, 해석 단계의 철자 오류가 개발 스터디 이름에 남지 않게 합니다.
...
### 이온화하는 약은 예측 하나로 용해도를 정하지 않습니다

산·염기 site가 있는 약물(구조 플래그로 계산)은 위장관 pH 1.2–6.8에서 용해도가 크게 달라집니다. 단일 ESOL 예측은 이 pH 의존성을 반영하지 못하므로, 이런 약은 BCS 잠정 용해도를 **"미정"**으로 두고 pH 1.2·4.5·6.8 평형용해도를 요청합니다(사유 꼬리표: `pH 의존(이온화)`). 공식 문헌 값이 있는 약물은 `literature_bcs.csv`의 한 행(등급: 문헌 표 수치, 구조로 대조)으로 잠정 판정을 채웁니다. 어느 경우든 BCS 등급 자체는 실측으로만 확정합니다.
...
측정값을 제출하면(데이터 요청 패널 또는 입력 에이전트) **근거 등급**(자체 실측 · 문헌 · 사용자 진술)과 함께 기록하고, intake·분자 계산은 다시 돌리지 않은 채 그 값에 의존하는 판정(페이즈 게이트 이후)만 다시 계산합니다(LLM 0회). 해설에는 제출값·근거 등급·닫힌 요청·다시 판정한 게이트가 뜹니다. 그다음 새 계획을 세워 **이전 계획과 서명을 비교합니다.**
...
```
0  입력 계약 검사     API 1행 · 요청 용량(유리염기) · 고정 부형제 · 라벨 1일 최대 용량
...
### 가장 먼저 — 이 후보가 요청한 그 약인가 (입력 계약 검사)

배합금기 규칙은 "이 조성이 위험한가"를 봅니다. 그 앞에 더 기본적인 질문이 있습니다 — **이 후보가 요청한 그 약, 그 용량, 그 고정 부형제인가.** 설계 AI는 요청 용량을 무시하거나(8 mg 요청에 10 mg), 주성분을 빠뜨리거나, 고정한 부형제를 뺄 수 있고, 배합금기 규칙은 그걸 모릅니다. 그래서 모든 규칙보다 먼저 결정론으로 대조합니다(`request_contract_rules.csv` · `max_daily_dose.csv`).

| 검사 | 기준 | 어기면 |
|---|---|---|
| API 존재 | API 행이 정확히 1개(role=api) | 반려 |
| 요청 용량 일치 | API 함량을 **유리염기로 환산**해 요청 1회 용량과 ±0.5% 이내 — 이름에 염 표기(besylate·HCl 등)가 있으면 염 기준으로 읽고 RDKit 분자량비로 환산 | 반려(용량이 입력되지 않았으면 "미검사"로 표시) |
| 고정 부형제 | 사용자가 고정한 부형제가 모두 들어 있다(부형제 사전으로 대조) | 반려 |
| LLM 추가 성분 | 고정 목록 밖 성분은 "LLM 추가 성분"으로 표시 | 표시만 |
| 1일 최대 용량 | 후보의 1회 함량(유리염기)이 허가 라벨의 1일 최대 용량을 넘지 않는다 — 약물은 이름이 아니라 **구조(InChIKey)로 대조**. 값은 FDA 라벨(DailyMed) 원문 문장과 함께 저장 | 반려(요청 용량 자체가 라벨 최대를 넘으면 재설계 없이 "제약 불가능") · 라벨이 없는 신규물질은 표시만 |

설계 에이전트는 요청 용량을 하드 제약으로 받고, 어기면 되돌림 표(BT030·BT031)에 따라 같은 전략으로 함량을 고쳐 다시 설계합니다.
...
### 역할이 아니라 부형제로 보는 배합비, 공정에 달린 위험은 DoE로

배합비 규칙은 부형제를 기능 역할(결합제 2–10%, 활택보조제 0.25–2% 등)로 묶어 판정하는데, 설계 AI가 붙인 역할만 보면 틀리는 부형제가 있습니다. 미결정셀룰로오스(MCC)는 결합제·희석제를 겸해 20–90%로 쓰고, 탈크의 활택 사용 범위는 1–10%로 콜로이드성 이산화규소와 다릅니다. 그래서 **출처가 있는 역할 매핑표**(`excipient_role_map.csv`, Handbook of Pharmaceutical Excipients)로 판정용 역할을 바꾸고 전용 범위(FR010·FR011)로 판정하며, 무엇을 바꿨는지 판정문에 남깁니다. 출처 없는 매핑 행은 판정에 쓰지 않습니다.

마그네슘 스테아레이트 과혼합(INC014)은 혼합 시간·전단력에 달린 **공정 의존 위험**이라, 윤활 혼합 시간이 정해지지 않은 후보 탐색 단계에서는 경고로 켜지 않고 **"DoE 요인 후보(윤활 혼합 시간)"**로 넘깁니다. 개발 스튜디오에서 요인으로 다룰 대상이라는 뜻입니다.

### 반려되면 어디로 돌아가나 — 사유별 되돌림

반려되면 무조건 처음부터 다시 설계하지 않습니다. 어느 단계로 돌아갈지는 코드가 아니라 `backtrack_transitions.csv`(16행)의 한 행이 정합니다 — 반려 판정(또는 측정 결과)이 행의 조건식에 맞으면 그 행의 **복귀 지점**으로 돌아가고, 행이 적은 **제약**(성분 제외 · 전략 제외 · 경로 제외 · 가족 요구 · 가족 감점)을 쌓습니다.
...
| 입력 계약 위반 (API·용량·고정 부형제) · 1일 최대 용량 초과 | 같은 전략으로 성분·함량만 수정 | — |
...
시간은 대회 API 모델 기준입니다. 무료 모델(Groq)은 분당 토큰 한도 때문에 같은 요청이 약 2–3배 걸립니다(데모 실행 결과 보고서의 무료 모델 시간과 같은 요청의 대회 API 재실행 시간 비교).

실행되는 동안 오른쪽에 "지금 어느 계층이 무엇을 왜 하는지" 해설이 단계별로 뜹니다. 모두 실제로 돌려서 어떤 경로를 밟는지 확인한 것입니다.

**시나리오 1 · 규칙이 AI를 막는 순간 (대회 API 약 30초).** 현장 사정 때문에 **유당을 반드시 써야 한다**고 못 박고 어린이용 플루옥세틴 정제(1회 10 mg)를 요청합니다.
...
**시나리오 2 · 요청에 따라 팀이 바뀐다.** 어린이용 바나나향 아세트아미노펜 정제(1회 160 mg)를 요청합니다. 어린이 안전 심사관이 생성되고 고령자 심사관은 만들어지지 않습니다. 나머지 심사관은 설계된 성분에 따라 달라집니다(예: 룰북 밖 조합이면 문헌 조사). 아세트아미노펜에 아민이 없어 유당 금기가 해당하지 않는다는 것도 구조 플래그에서 함께 확인됩니다.

**시나리오 3 · 값을 몰라도 후보부터, 갈리는 지점만 되묻는다 (대회 API 약 1분).** 성인용 이부프로펜 정제를 요청합니다(1회 투여량 200mg — 이부프로펜의 통상 단위 용량 — 을 입력해 둡니다. 용량이 있어야 DCS 분류에 필요한 용량–용해도 비를 계산할 수 있습니다). 페이즈 게이트가 **후보를 먼저 내고**, 판정이 갈리는 지점에서만 **데이터 요청** 패널에 실측을 요청합니다. 시연은 그 입력칸을 짚어 주고, 연구자가 가진 측정값을 넣어 제출하면 그래프를 다시 돌리지 않고 그 자리에서 재계산돼 남은 요청이 줄고 신뢰도 태그가 갱신됩니다. 측정값은 시스템이 대신 채우지 않습니다.
...
| 후보는 요청한 API·용량·고정 부형제를 지키고 라벨 1일 최대 용량을 넘지 않는다(입력 계약, 모든 규칙보다 먼저) | 용량을 무시하거나 주성분이 빠진 처방이 배합금기만 통과하면 합격으로 보인다 |
| 물성은 염을 벗긴 parent로 계산한다 | 짝이온 질량·극성이 섞여 경고가 잘못 켜진다 |
| 심사 점수는 검증된 DOI·PMID 인용이 있을 때만 합의에 들어간다 | 출처 없는 판단이 순위를 정한다 |
| LLM이 응답하지 않으면 결과를 만들지 않는다 | 가짜 처방·점수가 실제 판단처럼 보인다 |

LLM은 프로바이더에 묶이지 않습니다. 대회 제공 API(OpenAI Responses 호환, `gpt-5.6-sol`), Groq 무료 티어, Anthropic Claude를 같은 인터페이스로 감쌉니다. 화면의 입력 에이전트 머리에서 **모델을 고릅니다 — 기본은 무료 Groq**이고, **대회 API는 비밀번호로 접속한 경우에만** 고를 수 있습니다(게스트 접속은 Groq만). 이 권한은 허브가 세션을 보고 붙이는 헤더로 서버가 다시 검사합니다. 대회 API를 고른 호출은 팀 누적 한도 소진(403)이나 오류가 나면 같은 호출을 **무료 Groq로 자동 전환**합니다. 선택은 설계 실행·측정값 재계산·입력 에이전트·개발 스튜디오 호출에 같이 실립니다. 처방과 판정은 전부 **구조화 출력**으로 받습니다. Groq는 `max_tokens`도 분당 토큰 한도에 포함해 계산하고, 병렬 팬아웃이 동시에 한도를 때리면 호출이 거절됩니다. 그래서 클라이언트가 모델별 분당 토큰을 직접 회계하고, 예산이 있는 모델을 배정받을 때까지 순번을 기다립니다. 그래도 응답을 받지 못하면 **결과를 지어내지 않습니다** — 그 전략의 후보는 만들지 않고, 그 심사관은 "점수 없음"으로 표시하며, 순위는 실제로 매겨진 점수만으로 정합니다.
...
| POST | `/api/runs/{id}/measurements` | 데이터 요청 회신 `{measurements, grade: self_measured·literature·user_statement}` → 재계산(근거 등급 기록) |
...
└─ tests/
   ├─ (pytest 207개)            구조 패턴 진리표 · 검사 방향 · 페이즈 게이트 · 되돌림 · 입력 에이전트 가드레일 · 07_doe 룰 fixture · 골든 값 · study 흐름
...
- **라벨 1일 최대 용량 표**는 FDA 라벨(DailyMed)이 확인된 7개 약물만 담고 있습니다. 표에 없는 물질에는 규칙을 적용하지 않고 표시만 합니다. 요청 용량의 기준(유리염기/염)은 기본이 유리염기이고, 설계 AI가 API 이름에 염을 적었을 때만 염 기준으로 환산합니다.
- **문헌 BCS 표**는 한 행(로르녹시캄, 서지는 Crossref로 존재 확인·원문 쪽수 대조 전)입니다. FIP/WHO Biowaiver monograph 등으로 늘리는 일은 약학 팀 검토가 필요합니다.
- **니트로사민 위험 신호**(`has_nitrosatable_secondary_amine`)는 1,4-디히드로피리딘 고리 N–H에도 켜집니다. 고리 2차 아민 플래그에서는 비닐로그 아미드 N–H를 제외했지만, 니트로사민 위험평가는 규제 판단이라 문헌 근거를 확보할 때까지 그대로 둡니다.
- **INC014의 혼합 시간 상한**은 출처를 확보하지 못해 두지 않았습니다 — 시간이 정해지지 않았을 때 DoE 요인 후보로 넘기는 것까지만 합니다.
