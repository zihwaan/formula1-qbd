...
약대생 팀이 만든 기준서(v1.1)에 맞춰 구조 패턴을 82종으로 넓혔습니다. 핵심은 개수가 아니라 **판정을 세 등급으로 나눈 것**입니다.
...
기준서는 아민을 `1차 지방족`·`2차 지방족`·`3차 지방족`·`방향족`으로 나누라고 요구합니다. 그런데 배합 금기 규칙표는 여전히 `primary_amine`·`secondary_amine`이라는 이름으로 연결돼 있습니다. 이름만 바꾸면 **INC001~INC006이 조용히 발동하지 않게 됩니다.**
...
```
아세트아미노펜  → 아미드 검출, 아민 없음 → 반려 없음
플루옥세틴      → 2차 아민 검출        → 반려 1건 (INC002)
```
...
여기에 엔진 설정 표 2개(심사관 명단, 합의 가중치)와 **구조 패턴 목록(82종)** 을 런타임에 읽습니다. 구조 패턴 목록은 약대생 팀 기준서 v1.1을 표로 옮긴 것으로, 패턴마다 어떤 규칙을 발동시키는지·위험이 되려면 무슨 조건이 필요한지·무슨 시험으로 확인하는지가 함께 적혀 있습니다.
...
**동작하는 것** — 규칙표 27종이 여덟 개 검사 함수에 연결돼 순서대로 실행되고, 분자식에서 시작하는 계산 계층(구조 패턴 82종 + 분자 특성값 35종)이 판정의 입력을 만듭니다. 구조 경고가 확인시험 등급을 자동으로 올리고, 문헌 조사가 입력 단계에 붙어 있습니다. LangGraph 그래프가 설계·검증·심사·합의·반성을 잇고 심사관은 조건이 맞을 때만 생성됩니다. 실험 결과를 문장으로 넣으면 판독·판정·다음 실험 지시까지 돕니다. 회귀 테스트 54개와 브라우저 검증이 이를 고정합니다.
