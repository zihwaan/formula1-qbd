...
- **구조 플래그 레지스트리 v1.1** (`database/00_master/structural_flags_registry.csv`, 82 flags) — the pharmacy team's reference guide turned into data. Each row carries the full §3 contract: SMARTS, `alert_level` (fact / conditional_alert / hard_alert), `required_cofactors`, `confirmation_test`, `specificity`, `false_positive_notes`. Adding a flag is a row, not code. Two post-processors are declared per row: `ring_size:N` (β vs γ lactam — only the *ring* atoms of a match may be tested, the carbonyl oxygen is outside the ring and requiring it silences β-lactam entirely) and `count_ge:N` (polyphenol = 2+ phenols).
...
- **`formula/experimental_inputs.py` + `config/experimental_inputs.yaml`** — the optional
  "실험 데이터 입력" the user fills in *before* a run. Two non-obvious constraints:
  **(a) allowlist, not free-form** — these values are merged into the restricted-eval context, so
  an arbitrary key would let a user shadow `is_pediatric`/`flag` and bend the verdict; unknown or
  out-of-range keys are rejected and returned in `rejected_inputs` (never swallowed — a silent
  typo means no rule fires while the UI says "입력됨"). **(b) every field must name a real
  consumer** — `test_catalog_keys_are_used_by_rulebook_or_evidence` scans CSV parameter columns,
  CSV `condition_expression`, the manifest, and the evidence table; a field nobody reads is a typo.
  User values are seeded into the spec *before* `with_profile`, whose `setdefault` then can't
  overwrite them — 실측 > 추정 is enforced by assignment order, not by convention.
- **`formula/evidence/gate.py`** — the **Evidence Readiness Gate** (pre-experiment loop), added 2026-08-03 after review feedback that the repo implemented only the *post*-batch loop. Same data-driven contract as the rulebook: a requirement is one row of `database/reference/evidence_requirements.csv` (16 rows: 7 before_protocol / 4 parallel / 5 post_batch) with `applies_when` / `satisfied_when` evaluated through the same restricted eval (`checkers/applies_when.py`), extended with `flag()`, `prop()`, `has_measured()`, `ingredient()`, `role()` plus `process`/`strategy`/`is_salt`/`bcs_source`/`logs_status`.
  ...
  - `result_key` on a requirement is the canonical measurement that test produces. A numeric
    confirmation result is written into `spec.measured_params[result_key]` (`Run._apply_results`),
    so `satisfied_when` becomes true **on its own merit** rather than because a form was submitted —
    after that, deleting the confirmation record leaves the gap satisfied, exactly as if the value
    had been supplied up front. Failed results are never written (that's a negated premise, not evidence).
